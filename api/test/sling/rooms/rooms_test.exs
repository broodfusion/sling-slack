defmodule Sling.RoomsTest do
  ...
  describe "user_rooms" do
    ...
    test "list_user_rooms/0 returns all user_rooms" do
      ...
      assert Rooms.list_user_rooms() == [user_room]
    end

    test "get_user_room!/1 returns the user_room with given id" do
      user_room = user_room_fixture()
      assert Rooms.get_user_room!(user_room.id) == user_room
    end

    test "create_user_room/1 with valid data creates a user_room" do
      assert {:ok, %UserRoom{} = user_room} = Rooms.create_user_room(@valid_attrs)
    end

    test "create_user_room/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} = Rooms.create_user_room(@invalid_attrs)
    end

    test "update_user_room/2 with valid data updates the user_room" do
      user_room = user_room_fixture()
      assert {:ok, user_room} = Rooms.update_user_room(user_room, @update_attrs)
      assert %UserRoom{} = user_room
    end

    test "update_user_room/2 with invalid data returns error changeset" do
      user_room = user_room_fixture()
      assert {:error, %Ecto.Changeset{}} = Rooms.update_user_room(user_room, @invalid_attrs)
      assert user_room == Rooms.get_user_room!(user_room.id)
    end

    test "delete_user_room/1 deletes the user_room" do
      user_room = user_room_fixture()
      assert {:ok, %UserRoom{}} = Rooms.delete_user_room(user_room)
      assert_raise Ecto.NoResultsError, fn -> Rooms.get_user_room!(user_room.id) end
    end

    test "change_user_room/1 returns a user_room changeset" do
      user_room = user_room_fixture()
      assert %Ecto.Changeset{} = Rooms.change_user_room(user_room)
    end
  end
end
